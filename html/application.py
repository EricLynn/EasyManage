...
application.config['MYSQL_DATABASE_HOST'] = '54.227.125.23'
...
def getAllEntries():
    ...
    cur = mysql.get_db().cursor()
    command = []
    command.append('SELECT * entry ') #was, 'SELECT title, date_created, description, d_type FROM entry '     change back later...
    #command.append('WHERE organization_id=%s') no organizations yet
    #data = (session['org']) no current seesion org
